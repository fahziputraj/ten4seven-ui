...
| Web QR display, copy, or print                                   | `QrCode` with an opaque consumer-owned value, label, and accessible description                          | a scanner/camera flow, remote QR image service, or product-owned QR renderer |
| Nested hierarchy or resource-scope selection                     | `HierarchyPicker` with opaque node IDs, optional search, and consumer-owned selection persistence        | flat `Select`/`Combobox`, navigation-only `Sidebar`, or permission logic in the component |
| Visible action unavailable with a reason                         | Native-disabled `Button` + focusable `IconButton` reason trigger in `Tooltip` + linked helper text       | Tooltip directly around a disabled control, hidden action, or permission logic in `Button` |
| Comparable, sortable, selectable records                         | `DataTable`, `DataTableColumnPicker`, `BulkActionBar`                                                    | cards posing as a table                                            |
| Repeated editable line items with typed cells                    | `AdvancedDataGrid` for bounded text/number/currency/select editing and row state                          | stretching `DataTable` into a spreadsheet or putting validation, totals, or permissions in the grid |
