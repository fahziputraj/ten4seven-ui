...
export function CurrencyInput({ value, onValueChange, currency = "Rp", size = "md", invalid = false, disabled, style, ...rest }) {
  const [focus, setFocus] = React.useState(false);
  return (
    <div style={{ ...fieldShell(focus, invalid, disabled, heights[size]), ...style }}>
      <span style={{ paddingLeft: "var(--space-3)", font: "var(--type-label)", color: "var(--muted-foreground)", flex: "none" }}>{currency}</span>
      <input type="text" inputMode="numeric" disabled={disabled} aria-invalid={invalid || undefined}
        value={group(value)} placeholder="0"
        onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
        onChange={(e) => onValueChange?.(e.target.value.replace(/[^\d]/g, ""))}
        style={{ ...inner, padding: "0 var(--space-3)", textAlign: "right", fontVariantNumeric: "tabular-nums", fontWeight: "var(--weight-medium)" }} {...rest} />
    </div>
  );
}
