...
| Check                       | Actual result                                                                                                                                                                                                                                                                                                                      |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Deterministic direct routes | `/theme-studio`, `/tokens`, `/components`, `/icons`, `/recipes`, `/operations-tracker`, and `/ebook-store` each opened directly with the expected route title and H1. `/not-a-route` rendered the honest 404 surface.                                                                                                              |
| Refresh safety              | Theme Studio, Operations Tracker, and Ebook were opened directly, reloaded, then showed their expected H1 and title without console errors.                                                                                                                                                                                        |
| Shell separation            | Studio/Library/References are grouped on system surfaces. The production-shaped Operations Tracker/Ebook shells contain no `Fixture connected` copy, product-level view-state selector, or direct cross-reference utility. The fixed Reference Harness remains outside route shell markup and owns the operations fixture state.   |
| Operations entity-list      | `AppShell → Sidebar → PageHeader → KPICluster → MilestoneTracker → FilterToolbar → DataTable → Pagination → BulkActionBar → DetailDrawer` remains intact. Milestone selection/detail, row selection, bulk bar, detail drawer, loading fixture, ready reset, Escape closure, and focus return were exercised.                       |
...
| Mobile shell correction     | A Studio-only mobile `NavItem` rule had leaked into the Operations reference. It is now scoped to `.studio-sidebar`; ungrouped Sidebar items form a horizontal scroll rail at narrow widths instead of a tall unlabeled vertical stack.                                                                                            |

## Render evidence

All captures are first-viewport renders from the direct deterministic URLs, recaptured after neutralizing the local editorial cover lettering, normalizing operations bulk-selection copy, and retaining the mobile shell correction. `scrollWidth === clientWidth` for each document.

| Route      | Desktop `1440 × 900` | Mobile `390 × 844` | Mobile `360 × 800` |
| ---------- | -------------------- | ------------------ | ------------------ |
| Operations | `1425 / 1425`        | `375 / 375`        | `345 / 345`        |
| Ebook      | `1425 / 1425`        | `375 / 375`        | `345 / 345`        |

Artifacts:

- `render-evidence/operations-desktop-1440x900.png`
- `render-evidence/ebook-desktop-1440x900.png`
- `render-evidence/operations-mobile-390x844.png`
- `render-evidence/ebook-mobile-390x844.png`
- `render-evidence/operations-mobile-360x800.png`
